  ...
  inputSchema: {
    ...
    properties: {
      ...
      mode: {
        type: "string",
        description:
          "Travel mode (driving, walking, bicycling, transit, two_wheeler)",
  ...
  try {
    ...
    const formattedData = {
      ...
      price_level: placeData.priceLevel
        ? getPriceLevelValue(placeData.priceLevel)
        : 0,
      opening_hours: placeData.regularOpeningHours
        ? {
            ...
            periods:
              placeData.regularOpeningHours.periods?.map((period: any) => ({
                open: {
                  day: period.open?.day,
                  time: `${String(period.open?.hour).padStart(2, "0")}${String(
                    period.open?.minute
                  ).padStart(2, "0")}`,
                },
                close: period.close
                  ? {
                      ...
                      time: `${String(period.close.hour).padStart(
                        2,
                        "0"
                      )}${String(period.close.minute).padStart(2, "0")}`,
                    }
                  : null,
              })) || [],
          }
        ...
        placeData.photos?.map((photo: any) => ({
          ...
          html_attributions:
            photo.authorAttributions?.map(
              (attr: any) => `<a href="${attr.uri}">${attr.displayName}</a>`
            ) || [],
        })) || [],
      // Additional attributes
      ...
      payment_options: placeData.paymentOptions
        ? {
            accepts_credit_cards:
              placeData.paymentOptions.acceptsCreditCards || false,
            accepts_debit_cards:
              placeData.paymentOptions.acceptsDebitCards || false,
            accepts_cash_only:
              placeData.paymentOptions.acceptsCashOnly || false,
      ...
      accessibility_options: placeData.accessibilityOptions
        ? {
            wheelchair_accessible_entrance:
              placeData.accessibilityOptions.wheelchairAccessibleEntrance ||
              false,
            wheelchair_accessible_parking:
              placeData.accessibilityOptions.wheelchairAccessibleParking ||
              false,
            wheelchair_accessible_restroom:
              placeData.accessibilityOptions.wheelchairAccessibleRestroom ||
              false,
            wheelchair_accessible_seating:
              placeData.accessibilityOptions.wheelchairAccessibleSeating ||
              false,
          }
        : {},
    };
    ...
    return {
      content: [
        {
          type: "text",
          text: `Place details request failed: ${
...
async function handleDistanceMatrix(
  ...
  if (response.status !== 200 || (data[0] && data[0].error)) {
    return {
      content: [
        {
          type: "text",
          text: `Distance matrix request failed: ${
            data[0]?.error?.message || response.statusText || "Unknown error"
          }`,
        },
      ],
      isError: true,
  ...
  const resultMatrix = Array(origins.length)
    .fill(null)
    .map(() => ({
    ...
    resultMatrix[originIdx].elements[destIdx] = {
      ...
      duration: {
        ...
        value: item.distanceMeters
          ? parseInt(item.duration.replace("s", ""))
          : 0,
      },
      distance: {
        text: item.distanceMeters
...
async function handleDirections(
  ...
  const response = await fetch(url.toString(), {
    ...
    headers: {
      ...
      "X-Goog-Api-Key": GOOGLE_MAPS_API_KEY,
      "X-Goog-FieldMask":
        "routes.description,routes.distanceMeters,routes.duration,routes.staticDuration,routes.legs.steps.distanceMeters,routes.legs.steps.staticDuration,routes.legs.steps.navigationInstruction,routes.legs.steps.travelAdvisory",
    },
    body: JSON.stringify(requestBody),
  });
  ...
  if (response.status !== 200 || data.error) {
    return {
      content: [
        {
          type: "text",
          text: `Directions request failed: ${
            data.error?.message || response.statusText || "No routes found"
          }`,
        },
      ],
      isError: true,
                ...
                steps:
                  route.legs?.[0]?.steps?.map((step: any) => ({
                    instructions:
                      ...
                      step.travelAdvisory?.text?.text ||
                      "",
                    distance: {
                      text: step.distanceMeters
                        ? `${(step.distanceMeters / 1000).toFixed(1)} km`
                        : "Unknown distance",
                      value: step.distanceMeters,
                    },
                    duration: {
                      text: step.staticDuration
                        ? step.staticDuration.replace("s", "")
                        : "Unknown duration",
                      value: step.staticDuration
                        ? parseInt(step.staticDuration.replace("s", ""))
                        : 0,
                    },
